...
- extract repeated Flight IPC framing when it is used by several operations with the same reason to change;
- create an adapter when project code must be isolated from an external Arrow or gRPC representation;
- create a client factory when channel creation requires lifecycle management and several credential modes;
- create a registry when there are several protocol handlers selected at runtime.
